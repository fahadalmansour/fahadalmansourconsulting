...
function fsc_get_current_language() {
	if ( isset( $_GET['lang'] ) ) {
		...
		if ( in_array( $raw, array( 'ar', 'en' ), true ) ) {
...
function fsc_customize_social( $wp_customize ) {
	...
	// LinkedIn
	$wp_customize->add_setting( 'fsc_linkedin', array( 'sanitize_callback' => 'esc_url_raw' ) );
	...
	// Twitter / X
	$wp_customize->add_setting( 'fsc_twitter', array( 'sanitize_callback' => 'esc_url_raw' ) );
	$wp_customize->add_control(
		'fsc_twitter',
		array(
			'type'    => 'url',
	...
	// YouTube
	$wp_customize->add_setting( 'fsc_youtube', array( 'sanitize_callback' => 'esc_url_raw' ) );
	$wp_customize->add_control(
		'fsc_youtube',
		array(
			'type'    => 'url',
...
function fsc_customize_register( $wp_customize ) {
	...
	$wp_customize->add_setting( 'fsc_ga_id', array( 'sanitize_callback' => 'sanitize_text_field' ) );
	$wp_customize->add_control(
		'fsc_ga_id',
		array(
			'type'    => 'text',
	...
	$wp_customize->add_setting( 'fsc_gtm_id', array( 'sanitize_callback' => 'sanitize_text_field' ) );
	$wp_customize->add_control(
		'fsc_gtm_id',
		array(
			'type'    => 'text',
	...
	// 3. Meta Pixel ID (New)
	$wp_customize->add_setting( 'fsc_pixel_id', array( 'sanitize_callback' => 'sanitize_text_field' ) );
	$wp_customize->add_control(
		'fsc_pixel_id',
		array(
			'type'    => 'text',
	...
	$wp_customize->add_setting( 'fsc_header_scripts', array( 'sanitize_callback' => 'fsc_sanitize_scripts' ) );
	$wp_customize->add_control(
		'fsc_header_scripts',
		array(
			'type'        => 'textarea',
...
function fsc_schema_markup() {
	...
	$organization_schema = array(
		...
		'areaServed'                => array(
			array(
				'@type' => 'Country',
				'name'  => $is_rtl ? 'المملكة العربية السعودية' : 'Saudi Arabia',
			),
			array(
				'@type' => 'Country',
				'name'  => $is_rtl ? 'الإمارات العربية المتحدة' : 'United Arab Emirates',
			),
			array(
				'@type' => 'Country',
				'name'  => $is_rtl ? 'الكويت' : 'Kuwait',
			),
			array(
				'@type' => 'Country',
				'name'  => $is_rtl ? 'البحرين' : 'Bahrain',
			),
			array(
				'@type' => 'Country',
				'name'  => $is_rtl ? 'قطر' : 'Qatar',
			),
			array(
				'@type' => 'Country',
				'name'  => $is_rtl ? 'عُمان' : 'Oman',
			),
			array(
				'@type' => 'GeoShape',
				'name'  => $is_rtl ? 'دول مجلس التعاون الخليجي' : 'GCC Countries',
			),
		),
		'serviceType'               => array(
			'IT Consulting',
		...
		'paymentAccepted'           => $is_rtl
			? array( 'تحويل بنكي', 'حوالة مصرفية' )
			: array( 'Bank Transfer', 'Wire Transfer' ),
		'currenciesAccepted'        => array( 'SAR', 'USD', 'AED' ),
	...
	if ( is_front_page() ) {
		...
		$page_schema = array(
			...
			'mainEntity'  => array( '@id' => $site_url . '#organization' ),
		);
	} elseif ( is_page( 'services' ) ) {
...
function fsc_get_page_templates() {
	return array(
		...
		'contact'      => array(
			'title'    => __( 'Contact', 'fsc' ),
			'template' => 'page-contact.php',
		),
		'services'     => array(
			'title'    => __( 'Services', 'fsc' ),
			'template' => 'page-services.php',
		),
		'how-we-work'  => array(
			'title'    => __( 'How We Work', 'fsc' ),
			'template' => 'page-how-we-work.php',
		),
		'privacy'      => array(
			'title'    => __( 'Privacy Policy', 'fsc' ),
			'template' => 'page-privacy.php',
		),
		'cookies'      => array(
			'title'    => __( 'Cookie Policy', 'fsc' ),
			'template' => 'page-cookies.php',
		),
		'terms'        => array(
			'title'    => __( 'Terms of Use', 'fsc' ),
			'template' => 'page-terms.php',
		),
		'disclaimer'   => array(
			'title'    => __( 'Disclaimer', 'fsc' ),
			'template' => 'page-disclaimer.php',
		),
		'disclosure'   => array(
			'title'    => __( 'Disclosure', 'fsc' ),
			'template' => 'page-disclosure.php',
		),
	);
}
...
function fsc_fix_templates_tool() {
	...
	if ( ! isset( $_GET['fsc_fix_templates'] ) ) {
		return;
	}
