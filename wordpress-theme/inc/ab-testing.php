...
class FSC_AB_Testing {
	 ...
	 */
	private static $visitor_variants = array();
	...
	public static function init() {
		...
		// Register shortcodes
		add_shortcode( 'fsc_ab_test', array( __CLASS__, 'shortcode_ab_test' ) );
		add_shortcode( 'fsc_ab_variant', array( __CLASS__, 'shortcode_ab_variant' ) );

		// Track conversions via AJAX
		add_action( 'wp_ajax_fsc_ab_conversion', array( __CLASS__, 'track_conversion' ) );
		add_action( 'wp_ajax_nopriv_fsc_ab_conversion', array( __CLASS__, 'track_conversion' ) );
	...
	public static function register_test( $test_id, $config ) {
		self::$tests[ $test_id ] = wp_parse_args(
			$config,
			array(
				'name'     => $test_id,
				'variants' => array(
...
// Initialize A/B Testing
add_action( 'init', array( 'FSC_AB_Testing', 'init' ) );
